...
namespace Marreira\MCP_Bricks\MCP;

use Marreira\MCP_Bricks\Auth\Rest_Guard;
use Marreira\MCP_Bricks\MCP\Tools\Page_Tools;
use Marreira\MCP_Bricks\MCP\Tools\Template_Tools;
use Marreira\MCP_Bricks\MCP\Tools\Element_Tools;
use Marreira\MCP_Bricks\MCP\Tools\Style_Tools;
use Marreira\MCP_Bricks\MCP\Tools\Util_Tools;
use WP_REST_Request;
...
class MCP_Server {
	...
	public function hide_from_index( $response ) {
		...
		if ( isset( $data['routes'] ) && is_array( $data['routes'] ) ) {
			foreach ( array_keys( $data['routes'] ) as $route ) {
				if ( 0 === strpos( ltrim( $route, '/' ), MMB_REST_NAMESPACE ) ) {
	...
	/**
	 * Esconde o conteudo do indice do proprio namespace (/wp-json/marreira-mcp/v1).
	 *
	...
	public function hide_namespace_index( $response, $namespace ) {
		if ( MMB_REST_NAMESPACE === $namespace ) {
		...
		switch ( $method ) {
			case 'initialize':
				return $this->rpc_result(
					$id,
					array(
						'protocolVersion' => MMB_MCP_PROTOCOL_VERSION,
						'capabilities'    => array(
							'tools' => array( 'listChanged' => false ),
						),
						'serverInfo'      => array(
							'name'    => 'MarreiraMCP Bricks',
							'version' => MMB_VERSION,
						),
					)
				);
