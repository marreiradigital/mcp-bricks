...
namespace Marreira\MCP_Bricks\Auth;
...
class Rest_Guard {
	...
	public static function check( WP_REST_Request $request ) {
		...
		if ( ! empty( $settings['https_only'] ) && ! is_ssl() ) {
			return new WP_Error(
				'mmb_https_required',
				__( 'O endpoint MCP exige HTTPS.', 'marreira-mcp-bricks' ),
				array( 'status' => 403 )
			);
		}
		...
		if ( '' === $token ) {
			return new WP_Error(
				'mmb_missing_token',
				__( 'Token Bearer ausente.', 'marreira-mcp-bricks' ),
				array( 'status' => 401 )
			);
		}
		...
		if ( ! Token_Manager::validate( $token ) ) {
			return new WP_Error(
				'mmb_invalid_token',
				__( 'Token invalido.', 'marreira-mcp-bricks' ),
				array( 'status' => 403 )
			);
		}
	...
	private static function rate_limit( $token, $settings ) {
		...
		$key   = 'mmb_rl_' . substr( hash( 'sha256', $token ), 0, 20 );
		$count = (int) get_transient( $key );

		if ( $count >= $max ) {
			return new WP_Error(
				'mmb_rate_limited',
				__( 'Limite de requisicoes excedido. Tente novamente em instantes.', 'marreira-mcp-bricks' ),
				array( 'status' => 429 )
			);
		}
